...
myStartupHook = do
  ...
  --spawnOnce "xargs xwallpaper --stretch < ~/.config/wallpapers/w1.jpg"
...
myWorkspaces = [" dev ", " www ", " sys ", " doc ", " teams ", " chat ", " mus ", " games ", " rand "]
  ...
  let subKeys str ks = subtitle' str : mkNamedKeymap c ks
   in subKeys
        ...
        [ ("M-C-r", addName "Recompile XMonad" $ spawn "xmonad --recompile"),
          ...
          ("M-<Space>", addName "Run prompt" $ spawn "rofi -combi-modi window,drun,ssh -theme solarized -font 'SauceCodePro Nerd Font Mono 10' -show combi"),
        ...
        ^++^ subKeys
          ...
          [ ("M-<Tab>", addName "Switch to next layout" $ sendMessage NextLayout),
            ("M-S-<Return>", addName "Toggle noborders/full" $ sendMessage (MT.Toggle NBFULL) >> sendMessage ToggleStruts)
          ]
        -- Window resizing
        ^++^ subKeys
          "Window resizing"
	    ...
	    -- Does not currently work no dm script
